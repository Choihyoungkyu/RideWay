...
class BoardUpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mEditor.html = intent.getStringExtra("content")
    ...
    private fun updateBoard(){
        if(binding.title.text.isNullOrBlank()){
            Toast.makeText(this@BoardUpdateActivity,"제목을 입력해주세요",Toast.LENGTH_SHORT).show()
        ...
        var boardNum = intent.getIntExtra("boardNum",0)
        ...
        requestBody["board_id"] = RequestBody.create(MediaType.parse("text/plain"),boardNum.toString())
        ...
        retrofitAPI.updateBoard(requestBody,images).enqueue(object : Callback<Void>{
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                println(response)
                if(response.isSuccessful){
                    Toast.makeText(this@BoardUpdateActivity,"게시글 수정 성공",Toast.LENGTH_SHORT).show()
                    finish()
                }else{
                    Toast.makeText(this@BoardUpdateActivity,"게시글 수정 실패",Toast.LENGTH_SHORT).show()
                }
            }
    ...
    fun getPath(path: Uri,context : Context): String {
