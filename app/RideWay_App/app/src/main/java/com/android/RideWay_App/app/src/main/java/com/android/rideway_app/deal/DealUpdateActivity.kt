...
class DealUpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.etProduct.setText(intent.getStringExtra("product"))
        binding.dealEditor.html = intent.getStringExtra("content")
        ...
        binding.btnImageUpload.setOnClickListener {
            if(uriList.size >= 5){
                Toast.makeText(this@DealUpdateActivity,"이미지는 최대 5장 선택 가능합니다.",Toast.LENGTH_SHORT).show()
            }else {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode == REQUEST_CODE){
            if(data == null){ //이미지를 선택하지 않은 경우
                Toast.makeText(this@DealUpdateActivity,"이미지를 선택해주세요.",Toast.LENGTH_SHORT).show()
            }else{
                if(data.clipData == null){ // 이미지를 하나만 선택한 경우
                    ...
                    if(uriList.size + clipData!!.itemCount > 5){ //선택된 이미지가 총 5장 이상이 될 경우
                        Toast.makeText(this@DealUpdateActivity,"이미지가 5장을 초과합니다.",Toast.LENGTH_SHORT).show()
                    }else{
    ...
    private fun updateDeal(){
        ...
        val dealId = intent.getLongExtra("dealId",0).toString()
        ...
        requestBody["deal_id"] = RequestBody.create(MediaType.parse("text/plain"),dealId)
        ...
        retrofitAPI.updateDeal(requestBody,images).enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if(response.isSuccessful){
                    Toast.makeText(this@DealUpdateActivity, "등록 완료", Toast.LENGTH_SHORT).show()
                    finish()
                }else{
                    Toast.makeText(this@DealUpdateActivity,"등록 실패",Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<Void>, t: Throwable) {
                t.printStackTrace()
                Toast.makeText(this@DealUpdateActivity,"네트워크 오류",Toast.LENGTH_SHORT).show()
            }
