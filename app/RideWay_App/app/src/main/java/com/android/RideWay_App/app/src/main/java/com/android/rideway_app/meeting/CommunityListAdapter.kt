...
class CommunityListAdapter(val data : MutableList<CommunityListResponse.Content>,val channel : CommunityChannel) : RecyclerView.Adapter<CommunityListAdapter.MyViewHolder>() {
    private val retrofitAPI = RetrofitClient.getInstance().create(CommunityService::class.java)
    inner class MyViewHolder(binding : MeetingListBinding) : RecyclerView.ViewHolder(binding.root){
        var isSignUp = true
        var bind = binding
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        var binding : MeetingListBinding = MeetingListBinding.inflate(LayoutInflater.from(parent.context),parent,false)
